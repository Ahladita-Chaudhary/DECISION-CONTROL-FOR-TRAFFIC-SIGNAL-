...
Reference: Section 1.5, \textit{Application: A Randomized Min-Cut Algorithm} in \textit{Probability and Computing} (2nd Edition).
...
\textbf{Definitions:}
\begin{itemize}
    \item \textbf{Capacity of a cut:} The sum of the capacity of the edges in the cut that are oriented from a vertex $\in X$ to a vertex $\in Y$.
...
\begin{itemize}
    \item Either there is a minimum cut of $G$ that separates $s$ and $t$, in which case a minimum $s$--$t$ cut of $G$ is a minimum cut of $G$;
